...
class Compose:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        print("Before resizing - Image shape:", img.shape, "Mask shape:", mask.shape)

        for transform in self.transforms:
            img, mask = transform(img, mask)
        print("after resizing - Image shape:", img.shape, "Mask shape:", mask.shape)
...
class Normalize:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        if img.shape[0] == 1:
            img = img.repeat(3, 1, 1)  # Convert single-channel image to three-channel
        img = img.float() / 255  # Normalize the pixel values to the range [0, 1]
        #print(img.shape)
        #print(img)
...
class Pad:
    def __init__(self, size: Union[List[int], Tuple[int], int], seg_fill: int = 0) -> None:
        ...
        print("Fill Value init 999999999999999999:", size)
        print("Padding init:", seg_fill)

    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        padding = (0, 0, self.size[1]-img.shape[2], self.size[0]-img.shape[1])
        # Print relevant values for debugging #TODO:DELETE PRINT
        print("Fill Value call????:", float(self.seg_fill))
        print("Padding call??????/:", padding)
...
class RandomResizedCrop:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if img.shape[1:] != self.size:
            ...
            # Print relevant values for debugging #TODO:DELETE
            print("Fill Value randon_rrsize_crop:", float(self.seg_fill))
            print("Padding randon_rrsize_crop:", padding)
...
def get_train_augmentation(size: Union[int, Tuple[int], List[int]], seg_fill: int = 0):
    return Compose([
        ...
        #ResizePad(size, seg_fill=seg_fill),  # TODO: ADDED. DELETE?
